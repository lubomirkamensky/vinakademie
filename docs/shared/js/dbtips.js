...
DbTips = {
  ...
  init : function() {
    ...
    DbTips._sqlQueries =  [];
    DbTips._sqlQueries.push(['select A,M where not A starts with "_" #where# label A "nazev",M "lokalita"',['B','M','T'],'select A, M where not A starts with "_" label A "Typ",M "Lokalita"','Atraktivity'])
    DbTips._sqlQueries.push(['select A,Q where not A starts with "_" #where# label A "nazev",Q "lokalita"',['B','Q','V'],'select L, K where not L starts with "_" label L "Typ",K "Lokalita"','Akce'])
    DbTips._sqlQueries.push(['select A,J where not A starts with "_" #where# label A "nazev",J "lokalita"',['B','J','Q'],'select C, K where not C starts with "_" label C "Typ",K "Lokalita"','Aktivity'])
    DbTips._sqlQueries.push(['select A where not A starts with "_" #where# label A "nazev"',['B','J','C'],'select E where not E starts with "_" label E "Typ"','Programy'])
    DbTips._sqlQueries.push(['select A where not A starts with "_" #where# label A "nazev"',['B','I','C'],'select G where not G starts with "_" label G "Typ"','Trasy'])
  ...
  _drawDbTips : function(response) {
    ...
    for (var i = 0; i < dataTable.getNumberOfRows(); i++) {
     ...
     if (value) {
        html.push('<tr><td><a href="dbdetail.htm?gid='+ DbTips._gid + params +'" target="db">'+ title +'</a></td></tr>');
     }
    }
    html.push('</table>');
