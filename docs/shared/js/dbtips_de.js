...
DbTips = {
  ...
  init : function() {
    ...
    DbTips._sqlQueries =  [];
    DbTips._sqlQueries.push(['select Z,M where not A starts with "_" #where# label Z "nazev",M "lokalita"',['B','M','T'],'select A, M where not A starts with "_" label A "Typ",M "Lokalita"','Atraktivity'])
    DbTips._sqlQueries.push(['select AC,Q where not A starts with "_" #where# label AC "nazev",Q "lokalita"',['B','Q','V'],'select L, K where not L starts with "_" label L "Typ",K "Lokalita"','Akce'])
    DbTips._sqlQueries.push(['select V,J where not A starts with "_" #where# label V "nazev",J "lokalita"',['B','J','Q'],'select C, K where not C starts with "_" label C "Typ",K "Lokalita"','Aktivity'])
    DbTips._sqlQueries.push(['select U where not A starts with "_" #where# label U "nazev"',['B','J','C'],'select E where not E starts with "_" label E "Typ"','Programy'])
    DbTips._sqlQueries.push(['select T where not A starts with "_" #where# label T "nazev"',['B','I','C'],'select G where not G starts with "_" label G "Typ"','Trasy'])
  ...
  _drawDbTips : function(response) {
    ...
    for (var i = 0; i < dataTable.getNumberOfRows(); i++) {
     ...
     if (value) {
        html.push('<tr><td><a href="dbdetail_de.htm?gid='+ DbTips._gid + params +'" target="db">'+ title +'</a></td></tr>');
     }
    }
    html.push('</table>');
