...
DbMap = {
  ...
  init : function() {
    ...
    DbMap._sqlQueries =  [];
    DbMap._sqlQueries.push(['SELECT Z,AE,N,O,AC,B WHERE N contains "48." AND O contains "16."','select A, B, P where not A starts with "_"','Touristenziele']);
    DbMap._sqlQueries.push(['SELECT AC,AI,R,S,AG,B WHERE R contains "48." AND S contains "16."','select L, M, V where not A starts with "_"','Kalender der Veranstaltungen']);
    DbMap._sqlQueries.push(['SELECT V,Z,K,L,X,B WHERE K contains "48." AND L contains "16."','select C, D, R where not A starts with "_"','Aktivitäten']);
    DbMap._sqlQueries.push(['','']);
    DbMap._sqlQueries.push(['','']);
    DbMap._sqlQueries.push(['SELECT A,AX,AH,AI,AR,B WHERE AH contains "48." AND AI contains "16."','select N, O, X where not A starts with "_"','Unterkunft']);
    DbMap._sqlQueries.push(['SELECT A,V,M,N,S,B WHERE M contains "48." AND N contains "16."','select I, J, T where not A starts with "_"','Sonstige Dienstleistungen']);
