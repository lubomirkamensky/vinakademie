...
DbList = {
  ...
  init : function() {
    ...
    DbList._sqlQueries.push(['select A,O,Q,D,C where not A starts with "_" #where# label A "nazev",Q "lokalita",O "popis",D "termin",C "kde"',['B','Q','V'],'select L, K where not L starts with "_" label L "Typ",K "Lokalita"','Akce'])
    ...
    if (where != '')  {
      if (DbList._gid <5 ) {where = where + ' and A is not null and A <> ""'}
