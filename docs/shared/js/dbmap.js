...
DbMap = {
  ...
  _drawCheckForm : function(response) {
      ...
      for (var i = 0; i < rowsnum; i++) {
        if (data.getValue(i, 0) != "" && data.getValue(i, 1) != "") {
          ...
          html = html + '<input type="checkbox" id="' + data.getValue(i, 1) + 'box" onclick="DbMap.boxclick(this,\'' + data.getValue(i, 1) + '\')" /> ' + data.getValue(i, 0) + '<br/>';
        }
      }
  ...
  _drawMarkers : function(response) {
    ...
    for (var i = 0; i < rowsnum; i++) {
      ...
      //alert(name);
      var html = "<div style='width:240px'><b>"+name+"<\/b><p>"+description+"<\/p><p>"+address+"<\/p><\/div><p><a href='dbdetail.htm?gid="+DbMap._gid+"&nazev="+name+"' target='_blank'>Více informací (otevře nové okno)<\/a><\/p>";
      var category = data.getValue(i, 5).split(", ");
      // create the marker
      //alert(name);
      for(z = 0; z < category.length; z++){
        //alert(name + ", " + category[z] + ", " + DbMap._categD[category[z]]);
        //alert(name);
  ...
  init : function() {
    ...
    DbMap._sqlQueries =  [];
    DbMap._sqlQueries.push(['SELECT A,AE,N,O,K,B WHERE N contains "48." AND O contains "16."','select A, B where not A starts with "_"','Atraktivity']);
    DbMap._sqlQueries.push(['SELECT A,AI,R,S,O,B WHERE R contains "48." AND S contains "16."','select L, M where not A starts with "_"','Akce']);
    DbMap._sqlQueries.push(['SELECT A,Z,K,L,M,B WHERE K contains "48." AND L contains "16."','select C, D where not A starts with "_"','Aktivity']);
    DbMap._sqlQueries.push(['','']);
    DbMap._sqlQueries.push(['','']);
    DbMap._sqlQueries.push(['SELECT A,AX,AH,AI,J,B WHERE AH contains "48." AND AI contains "16."','select N, O where not A starts with "_"','Ubytování']);
    DbMap._sqlQueries.push(['SELECT A,V,M,N,J,B WHERE M contains "48." AND N contains "16."','select I, J where not A starts with "_"','Ostatní služby']);
